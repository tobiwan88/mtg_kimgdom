...
abilities.  The setup card defines how many characters of each type are
used per player count (3–12 players).
...
# tips           – list of tip bullet strings from the PDF tip card
# clarifications – list of rules clarification strings (may be empty)
# ---------------------------------------------------------------------------
...
#
# Built incrementally from the base set (King, Assassin, Bandit):
# 3p  — 1 King, 0 Knights, 1 Assassin, 1 Bandit, 0 Renegades
# 4p  — +Knight
# 5p  — +Renegade
# 6p  — +2nd Knight
# 7p  — +2nd Assassin
...
def make_distribution(n: int) -> list[str]:
    ...
    Args:
        n: Total number of players (3–12).
    ...
    if n not in _DISTRIBUTIONS:
        raise ValueError(
            f"Advanced Kingdoms supports {MIN_PLAYERS}–{MAX_PLAYERS} players, got {n}."
        )
